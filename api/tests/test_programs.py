...
from sqlmodel import SQLModel, create_engine, Session, StaticPool # Ważny import StaticPool
...
async def test_program_ownership_in_memory(token_user_a, token_user_b):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        create_res = await ac.post("/api/programs", json={
            "name": "Memory Program",
            "duration_days": 1,
            "description": "Test",
            "language": "pl",
        ...
        edit_res = await ac.put(f"/api/programs/{prog_id}", 
                               json={"name": "Evil Change"}, 
                               cookies={"access_token": token_user_b})
        
        assert edit_res.status_code == 403
        assert edit_res.json()["detail"] == "Not enough permissions to edit this program"

        delete_res = await ac.delete(f"/api/programs/{prog_id}", 
                                    cookies={"access_token": token_user_a})
