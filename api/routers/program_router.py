...
def create_program_day(day_in: ProgramDayCreate, session: SessionDep):
    ...
    if not program:
        raise HTTPException(
            status_code=404, detail=f"Program with id={day_in.program_id} not found"
        )
...
def update_program_day(
    session: SessionDep, program_day_id: int, program_day_update: ProgramDayUpdate
):
    ...
    if program_day_update.program_id != None:
        ...
        if not program:
            raise HTTPException(
                status_code=404,
                detail=f"Program with id={program_day_update.program_id} not found",
            )
...
def link_activity_to_program_day(
    program_day_id: int, activity_id: int, session: SessionDep
):
    program_day = session.get(ProgramDay, program_day_id)
    ...
    link = ProgramDayActivityLink(
        program_day_id=program_day_id, activity_id=activity_id
    )
    session.add(link)
    session.commit()
    return {
        "ok": True,
        "message": f"Activity {activity_id} linked to ProgramDay {program_day_id}",
    }
